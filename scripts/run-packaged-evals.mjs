...
import { runPackagedDesktopEvalCase } from "./lib/packaged-eval-runner.js";
...
if (!existsSync(appPath)) {
  console.error(`[run-packaged-evals] Missing app bundle: ${appPath}`);
  process.exit(1);
}
...
try {
  ...
  console.error(`[run-packaged-evals] ${error instanceof Error ? error.message : String(error)}`);
  process.exit(1);
}
...
if (caseFiles.length === 0) {
  console.error(`[run-packaged-evals] No packaged eval cases found under ${casesRoot}`);
  process.exit(1);
}

mkdirSync(reportsRoot, { recursive: true });
console.log(`[run-packaged-evals] Using release config from ${releaseConfigSource}: ${releaseConfigPath}`);
...
for (const caseFile of caseFiles) {
  ...
  const reportPath = path.join(
    reportsRoot,
    `${path.basename(caseFile, ".case.json")}.report.json`,
  );

  try {
    console.log(`[run-packaged-evals] Running ${evalCase.id}`);
    await runPackagedDesktopEvalCase({
      evalCase,
      appPath,
      configPath: releaseConfigPath,
      reportPath,
    });
    console.log(`[run-packaged-evals] PASS ${evalCase.id}`);
  } catch (error) {
    failures += 1;
    console.error(`[run-packaged-evals] FAIL ${evalCase.id}`);
