...
function App() {
  ...
  const [activeView, setActiveView] = useState<"chat" | "diagnostics">("chat");
  ...
  return (
    <div className="flex h-screen bg-surface">
      ...
      <main className="flex flex-1 flex-col ml-[280px] bg-surface-cream">
        ...
        >
          <span className="text-sm font-medium text-ink-700">{activeView === "diagnostics" ? "Diagnostics" : activeSession?.title || "Letta"}</span>
        </div>
        ...
        {activeView === "chat" ? (
          ...
          <DiagnosticsPanel onBackToChat={() => setActiveView("chat")} />
        )}
      </main>
