...
const appHandlers = new Map<string, Function[]>();
const windows: Array<{
  closed: boolean;
...
const ipcMainHandleMock = vi.hoisted(() => vi.fn((key: string, handler: Function) => {
...
describe("main lifecycle", () => {
	...
	async function bootstrapMain(): Promise<{ app: any }> {
		await import("./main.ts");
		const { app } = await import("electron");
		return { app: app as any };
	}
